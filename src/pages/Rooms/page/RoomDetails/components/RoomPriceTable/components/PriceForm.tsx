    ...
    >
      {!data?.id && (
        <FormControl fullWidth sx={{ mb: 4 }}>
          ...
          <Select label="Temporada" defaultValue={undefined} {...formFields.seasonID}>
            {items.map((item) => {
              return (
                <MenuItem key={`select-season-${item?.label}-${item?.value}`} value={item?.value}>
                  {item?.label}
                </MenuItem>
              );
            })}
      ...
      <Button type="submit" variant="contained" sx={{ mt: 4, maxWidth: "50%", mx: "auto" }}>
        Guardar
      </Button>
    </Container>
  );
