...
const FormCalendar = ({
  ...
  const [dateRange, setDateRange] = useState<DateRange[]>(generateDateRange(dates));
  ...
  function onHandleCalendarChange(values: DateRange[]) {
    const start: string | null = values[0] ? format(values[0], DATE_FORMAT) : null;
    const end: string | null = values[1] ? format(values[1], DATE_FORMAT) : null;
  ...
  function handleContainerSize() {
    ...
    $formCalendar.forEach((calendar) => {
      if (calendar.parentElement && calendar.parentElement?.clientWidth < 1000) {
