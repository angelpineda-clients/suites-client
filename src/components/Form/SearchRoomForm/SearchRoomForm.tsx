...
const SearchRoomForm = ({ onSubmit }: ISearchRoomForm) => {
   ...
   */
  function handleAdultChange(event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) {
   ...
   * @return {*}
   */
  function handleAdultClick(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, value: string) {
    e.stopPropagation();
   ...
   * @return {*}
   */
  function handleChildrenClick(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, value: string) {
    e.stopPropagation();
    ...
    >
      <form id="home-form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="control-input">
          ...
          <TextField label="Ninos" type="number" value={children} sx={{ width: 80 }} />
          <button
